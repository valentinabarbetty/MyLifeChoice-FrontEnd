...
const FILAS = [
  ...
  { key: "aire",        fecha: "22/08", descripcion: "Aire acondicionado",   facturaIndex: 1 },
  { key: "desarrollo",  fecha: "02/04", descripcion: "Servicios desarrollo", facturaIndex: 2 },
  { key: "energia",     fecha: "08/12", descripcion: "Servicios energía",    facturaIndex: 3 },
];
...
const DESCRIPCIONES_FACTURAS = [
  {
    ...
    total: "$1,400",
    explicacion: "Esta factura muestra un alquiler de $1,200 más $200 de impuestos, dando un total de $1,400."
  },
  {
    ...
    total: "$3,000",
    explicacion: "Esta factura muestra un aire acondicionado de $2,300 más instalación de $700, dando un total de $3,000."
  },
  {
    ...
    total: "$3,700",
    explicacion: "Esta factura muestra desarrollo de aplicación de $3,500 más servidor de $200, dando un total de $3,700."
  },
  {
    ...
    cliente: "María Martínez",
    conceptos: [
      { nombre: "Consumo de energía", valor: "$700" }
    ],
    total: "$700",
    explicacion: "Esta factura muestra consumo de energía de $700, dando un total de $700."
  }
];
...
export default function ContaduriaGame({ onComplete }) {
  ...
  const prevFactura = () => goToFactura(currentFactura === 0 ? FACTURAS.length - 1 : currentFactura - 1);
  ...
  const handleCheck = () => {
    ...
    if (allCorrect) {
      ...
      const incorrectos = resultados
        .filter((r) => !r.correcto)
        .map((r) => `${r.descripcion}: debe ser $${VALORES_CORRECTOS[r.key].toLocaleString()}`)
        .join(". ");

      setAnnouncement(`Valores incorrectos. ${incorrectos}`);
  ...
  const getFacturaAriaLabel = () => {
    ...
    const conceptosTexto = desc.conceptos.map(c => `${c.nombre}: ${c.valor}`).join(", ");
    ...
    <div className="contaduriaOverlay">
      ...
      >
        <div id="contaduria-instructions" className="sr-only">
          Revisa las facturas del carrusel y ajusta los valores del libro
          de gastos usando los botones más y menos. Cuando todos los valores
          sean correctos, presiona Validar gastos.
        </div>

        <div className="contaduriaHeader">
          <div className="contaduriaIcon" aria-hidden="true">🕵️</div>
          <div>
            <h1
              id="contaduria-title"
              ref={titleRef}
            ...
            <p
              id="contaduria-subtitle"
              className="contaduriaSubtitle"
            >
        ...
        <div className="contaduriaContent">
          ...
          >
            <div className="contaduriaReceiptsHeader">
              ...
              <span className="contaduriaHint" aria-hidden="true">💡 Haz clic en las flechas</span>
            </div>
              ...
              >
                ...
                <div className="contaduriaImageOverlay" aria-hidden="true">
                  <div className="factura-info-overlay">
                    <strong>{DESCRIPCIONES_FACTURAS[currentFactura].titulo}</strong>
                    <span>Total: {DESCRIPCIONES_FACTURAS[currentFactura].total}</span>
                    <span>Cliente: {DESCRIPCIONES_FACTURAS[currentFactura].cliente}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="contaduriaProgress" aria-hidden="true">
              <div className="contaduriaProgressText">
                <span>Progreso</span>
                <span>{currentFactura + 1}/{FACTURAS.length}</span>
              </div>
            ...
            <div className="contaduriaTableBody" role="rowgroup">
              {FILAS.map((fila) => (
                <div
                  ...
                  role="row"
                  aria-current={fila.facturaIndex === currentFactura ? "true" : undefined}
                >
                  <span className="contaduriaFecha" role="cell">{fila.fecha}</span>
                  <span className="contaduriaDescripcion" role="cell">{fila.descripcion}</span>
